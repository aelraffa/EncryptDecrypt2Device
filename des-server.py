...
class DESServer:
    ...
    def handle_client(self, conn, addr):
        ...
        try:
            ...
            print(f"\n[TERIMA] Ciphertext (hex): {data}")
            
            # Dekripsi
            if self.key:
                try:
                    plaintext = self.des.decrypt(data, self.key)
                    print(f"[DEKRIPSI] Plaintext: {plaintext}")
                    
                    # Kirim konfirmasi
                    conn.send(f"Pesan diterima dan didekripsi: {plaintext}".encode('utf-8'))
                except Exception as e:
                    print(f"[ERROR] Gagal dekripsi: {e}")
                    conn.send(f"Error dekripsi: {e}".encode('utf-8'))
            else:
                print("[ERROR] Key belum diatur!")
                conn.send("Error: Key belum diatur di server".encode('utf-8'))
    ...
    def start(self):
        print("="*60)
        print("DES SERVER - DEVICE PENERIMA")
        print("="*60)
        
        # Set key
        self.key = input("\nMasukkan KEY (max 8 karakter): ").strip()
        if not self.key:
        ...
        try:
            ...
            server.listen(5)
            print(f"\n[*] Server berjalan di {self.host}:{self.port}")
            print(f"[*] Menunggu koneksi dari client...")
            print(f"[*] Key yang digunakan: {self.key}")
